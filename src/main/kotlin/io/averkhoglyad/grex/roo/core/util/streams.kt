package io.averkhoglyad.grex.roo.core.util
...
private class BatchedSpliterator<T>(private val root: Spliterator<T>, private val bufferSize: Int): Spliterator<List<T>> {
    ...
    override fun estimateSize(): Long {
        ...
        return if (rootSize == Long.MAX_VALUE) Long.MAX_VALUE else Math.ceil(rootSize / bufferSize.toDouble()).toLong()
    }

    override fun getExactSizeIfKnown(): Long {
        val rootExactSize = root.exactSizeIfKnown
        return if (rootExactSize < 0) -1 else Math.ceil(rootExactSize / bufferSize.toDouble()).toLong()
    }
